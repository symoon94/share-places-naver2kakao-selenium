...
class NaverCrawler:
    ...
    def openPage(self, url):
        ...
        with self.ghost.start() as session:
            page, extra_resources = session.open(url, timeout=300)
    ...
    def login(self, id, pw):
        ...
        self.ghost.evaluate("""
        (function() {        
          var innerDoc = document.getElementByClassName('login_form').contentWindow.document;
          innerDoc.getElementById('id').value = '%s';
          innerDoc.getElementById('pw').value = '%s';
          innerDoc.getElementsByClassName('btn_global')[0].click();
        })();
...
if __name__ == "__main__":
    crawler = NaverCrawler("vbfheu", "mm7868mm", False)
